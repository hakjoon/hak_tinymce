...
//---My Functions ----
function insertImage(obj,url,width,height,alt,longdesc) {
	//alert('<img src="'+url+'" width="'+width+'" height="'+height+'" alt="'+decodeURI(alt)+'" longdesc="'+decodeURI(longdesc)+'"');
	$('#src').set("value",url);
	$('#width').set("value",width);
	$('#height').set("value",height);
	$('#alt').set("value",decodeURI(alt));
	$("#longdesc").set("value",decodeURI(longdesc));
	$(obj.parentNode).addClass("active");
	$("#image_browse").find("div").not(obj.parentNode).removeClass("active");
}

function checkSelected(uri) {
	alert(uri);
}
// While loading
preinit();
